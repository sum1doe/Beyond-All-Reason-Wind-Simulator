...
class Simulator():
    ...
    def simulate_wind_drop(self, sim_time, min_wind, max_wind):
        ...
        data = np.zeros(total_sim_ticks//TICKS_PER_ITERATION+1)
        
        for tick in range(0, total_sim_ticks, TICKS_PER_ITERATION):
            wind_speed = self.update_wind(tick, min_wind, max_wind)
            data[tick//TICKS_PER_ITERATION] = wind_speed
            
        data = np.sort(data[data < avg_wind])
        
        index = int(len(data) *.9)
        ninety_percentile_lost = data[index] / (TICKS_PER_SEC / TICKS_PER_ITERATION)
